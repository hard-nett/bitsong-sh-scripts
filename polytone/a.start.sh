...
echo "Creating $BINARY instance for VAL1_A: home=$VAL1HOME | chain-id=$CHAINID_A | p2p=:$VAL1A_P2P_PORT | rpc=:$VAL1A_RPC_PORT | profiling=:$VAL1A_PPROF_PORT | grpc=:$VAL1A_GRPC_PORT"
echo "Creating $BINARY instance for VAL: home=$VAL2HOME | chain-id=$CHAINID_B | p2p=:$VAL1B_P2P_PORT | rpc=:$VAL1B_RPC_PORT | profiling=:$VAL1B_PPROF_PORT | grpc=:$VAL1B_GRPC_PORT"
...
sleep 1
$BIND --home $VAL1HOME config node tcp://localhost:$VAL1A_RPC_PORT
$BIND --home $VAL2HOME config node tcp:\/\/127.0.0.1:$VAL1B_RPC_PORT
sleep 1
...
# config.toml
sed -i.bak -e "s/^proxy_app *=.*/proxy_app = \"tcp:\/\/127.0.0.1:$VAL1A_PROXY_APP_PORT\"/g" $VAL1HOME/config/config.toml &&
sed -i.bak "/^\[rpc\]/,/^\[/ s/laddr.*/laddr = \"tcp:\/\/127.0.0.1:$VAL1A_RPC_PORT\"/" $VAL1HOME/config/config.toml &&
sed -i.bak "/^\[rpc\]/,/^\[/ s/address.*/address = \"tcp:\/\/127.0.0.1:$VAL1A_RPC_PORT\"/" $VAL1HOME/config/config.toml &&
sed -i.bak "/^\[p2p\]/,/^\[/ s/laddr.*/laddr = \"tcp:\/\/0.0.0.0:$VAL1A_P2P_PORT\"/" $VAL1HOME/config/config.toml &&
...
# val2
sed -i.bak -e "s/^proxy_app *=.*/proxy_app = \"tcp:\/\/127.0.0.1:$VAL1B_PROXY_APP_PORT\"/g" $VAL2HOME/config/config.toml &&
sed -i.bak "/^\[rpc\]/,/^\[/ s/laddr.*/laddr = \"tcp:\/\/127.0.0.1:$VAL1B_RPC_PORT\"/" $VAL2HOME/config/config.toml &&
sed -i.bak "/^\[rpc\]/,/^\[/ s/address.*/address = \"tcp:\/\/127.0.0.1:$VAL1B_RPC_PORT\"/" $VAL2HOME/config/config.toml &&
sed -i.bak "/^\[p2p\]/,/^\[/ s/laddr.*/laddr = \"tcp:\/\/0.0.0.0:$VAL1B_P2P_PORT\"/" $VAL2HOME/config/config.toml &&
...
sed -i.bak "/^\[api\]/,/^\[/ s/address.*/address = \"tcp:\/\/0.0.0.0:$VAL1A_API_PORT\"/" $VAL1HOME/config/app.toml &&
sed -i.bak "/^\[grpc\]/,/^\[/ s/address.*/address = \"localhost:$VAL1A_GRPC_PORT\"/" $VAL1HOME/config/app.toml &&
sed -i.bak "/^\[grpc-web\]/,/^\[/ s/address.*/address = \"localhost:$VAL1A_GRPC_WEB_PORT\"/" $VAL1HOME/config/app.toml &&
# val2
sed -i.bak "/^\[api\]/,/^\[/ s/minimum-gas-prices.*/minimum-gas-prices = \"0.0ubtsg\"/" $VAL2HOME/config/app.toml &&
sed -i.bak "/^\[api\]/,/^\[/ s/address.*/address = \"tcp:\/\/0.0.0.0:$VAL1B_API_PORT\"/" $VAL2HOME/config/app.toml &&
sed -i.bak "/^\[grpc\]/,/^\[/ s/address.*/address = \"localhost:$VAL1B_GRPC_PORT\"/" $VAL2HOME/config/app.toml &&
sed -i.bak "/^\[grpc-web\]/,/^\[/ s/address.*/address = \"localhost:$VAL1B_GRPC_WEB_PORT\"/" $VAL2HOME/config/app.toml &&
...
# init note
 ...
 sleep 5 
# init voice
 $BIND tx wasm i $POLYONE_VOICE_ID '{"proxy_code_id":'$POLYONE_PROXY_ID',"block_max_gas":100000000, "contract_addr_len":32}' --from $DEL --home $VAL1HOME --chain-id $CHAINID_A -y --fees 400000ubtsg --gas auto --gas-adjustment 1.3
  sleep 1 
 $BIND tx wasm i $POLYONE_VOICE_ID '{"proxy_code_id":'$POLYONE_PROXY_ID',"block_max_gas":100000000, "contract_addr_len":32}'  --from $DEL --home $VAL2HOME --chain-id $CHAINID_B -y --fees 400000ubtsg --gas auto --gas-adjustment 1.3
 sleep 5
# init tester
